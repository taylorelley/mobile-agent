...
BUILT_IN_TOOLS = [
    {
        ...
        "parameters": {
            "time": {
                ...
                "format": "HH:MM",
                "required": True,
                "description": "The alarm time in 24-hour format.",
            },
            "label": {
                "type": "string",
                "required": False,
    ...
    },
    {
        ...
        "parameters": {
            "filename": {
                "type": "string",
                "required": True,
                "description": "Name of the file to create (e.g. 'notes.txt', 'todo.md').",
            },
            "content": {
                "type": "string",
                "required": True,
            ...
            "directory": {
                "type": "string",
                "required": False,
                "description": "Optional directory path (e.g. 'documents', 'projects/myapp'). Defaults to root.",
            },
        },
    ...
    },
    {
        ...
        "parameters": {
            ...
            "mode": {
                "type": "string",
                "required": False,
                "description": "'overwrite' (default), 'append', or 'replace'. For 'replace', use find_text and replace_text params instead of content.",
            },
            "find_text": {
                "type": "string",
                "required": False,
                "description": "Text to find (used with mode='replace').",
            },
            "replace_text": {
                "type": "string",
                "required": False,
                "description": "Text to replace with (used with mode='replace').",
            },
            "directory": {
                "type": "string",
                "required": False,
    ...
    },
    {
        ...
        "parameters": {
            "directory": {
                "type": "string",
                "required": False,
                "description": "Optional directory to list. Defaults to root.",
            },
            "pattern": {
                "type": "string",
                "required": False,
                "description": "Optional filename pattern filter (e.g. '*.txt', '*.md').",
            },
        },
...
async def get_recent_conversations(session_id: str, limit: int = 3):
    convos = (
        await db.conversations.find({"session_id": session_id}, {"_id": 0})
        .sort("timestamp", -1)
        .limit(limit)
        .to_list(limit)
    )
...
async def get_relevant_facts(
    user_input: str, limit: int = 5, categories: Optional[List[str]] = None
):
    query = {}
    if categories:
    ...
    for f in facts:
        fact_words = set(f.get("value", "").lower().split()) | set(
            f.get("key", "").lower().split()
        )
...
def build_chat_prompt(
    ...
    if memory_snippets:
        memory_text = "\n## Relevant memories:\n" + "\n".join(
            [
                f"- [{f.get('category', 'general')}] {f.get('key', '')}: {f.get('value', '')}"
                for f in memory_snippets
            ]
        )
    history_text = ""
    ...
    if tool_name == "create_file":
        ...
        if existing:
            return {
                "tool": tool_name,
                "success": False,
                "error": f"File '{file_path}' already exists. Use edit_file to modify it.",
            }
        doc = {
            "id": str(uuid.uuid4()),
            "filename": filename,
        ...
        return {
            "tool": tool_name,
            "success": True,
            "result": f"File '{file_path}' created ({len(content)} chars)",
            "params": params,
        }
        ...
        return {
            "tool": tool_name,
            "success": True,
            "result": f"File '{file_path}' content:\n{file_doc['content']}",
            "params": params,
        ...
        return {
            "tool": tool_name,
            "success": True,
            "result": f"File '{file_path}' updated ({mode}, {len(new_content)} chars)",
            "params": params,
        }
        ...
        file_list = "\n".join(
            [
                f"- {f['path']} ({f.get('size_bytes', 0)} bytes, updated {f.get('updated_at', 'unknown')})"
                for f in files
            ]
        )
        return {
            ...
            "params": params,
            "files": [
                {"path": f["path"], "size_bytes": f.get("size_bytes", 0)} for f in files
            ],
        }
...
async def send_chat(msg: ChatMessage):
    ...
    chat_system, chat_user = build_chat_prompt(
        soul.get("content", DEFAULT_SOUL), memory_snippets, history, msg.message, tools
    )
    chat = LlmChat(
        api_key=api_key,
        session_id=f"chat-{msg.session_id}-{uuid.uuid4().hex[:8]}",
    ...
    if action_match:
        ...
        action_memory = await get_relevant_facts(
            msg.message, limit=2, categories=["schedule", "preference"]
        )
        action_system, action_user = build_action_prompt(
            action_description, tools, action_memory
        )
        action_chat = LlmChat(
            api_key=api_key,
            session_id=f"action-{uuid.uuid4().hex[:8]}",
            system_message=action_system,
        )
        ...
        confirm_chat = LlmChat(
            api_key=api_key,
            session_id=f"confirm-{uuid.uuid4().hex[:8]}",
            system_message=confirm_system,
        )
        ...
        if any(kw in user_lower for kw in keywords):
            ...
            clarify_response = await clarify_chat.send_message(
            ...
            if action_match2:
                ...
                action_chat = LlmChat(
                    api_key=api_key,
                    session_id=f"action-h-{uuid.uuid4().hex[:8]}",
                    system_message=action_system,
                )
                action_chat.with_model("openai", "gpt-4.1-mini")
                action_response = await action_chat.send_message(
                    UserMessage(text=action_user)
                )
                action_output = action_response
                ...
                try:
                    ...
                    if json_match:
                        ...
                        if "error" not in parsed:
                            ...
                            params = parsed.get("params", {})
                            if tool_name in FILE_TOOLS:
                                tool_result = await execute_file_operation(
                                    tool_name, params
                                )
                            else:
                                tool_result = simulate_tool_execution(
                                    tool_name, params, tools
                                )
                            tool_calls_str = json.dumps(
                                {"call": parsed, "result": tool_result}
                            )
                            confirm_system = "You are LobsterLite. Provide a brief confirmation of the action."
                            confirm_chat = LlmChat(
                                api_key=api_key,
                                session_id=f"confirm-h-{uuid.uuid4().hex[:8]}",
                                system_message=confirm_system,
                            )
                            confirm_chat.with_model("openai", "gpt-4.1-mini")
                            confirmation = await confirm_chat.send_message(
    ...
    return {
        ...
        "preamble": (
            chat_response[: action_match.start()].strip() if action_match else None
        ),
...
async def get_soul_endpoint():
    soul = await get_soul()
    return {
        "content": soul.get("content", DEFAULT_SOUL),
...
async def update_soul(update: SoulUpdate):
    now_str = datetime.now(timezone.utc).isoformat()
    await db.soul_config.update_one(
        {}, {"$set": {"content": update.content, "updated_at": now_str}}, upsert=True
    )
...
async def create_fact(fact: FactCreate):
    ...
    await db.facts.update_one(
        {"category": fact.category, "key": fact.key}, {"$set": doc}, upsert=True
    )
    return doc
...
async def list_conversations(session_id: Optional[str] = None, limit: int = 50):
    ...
    convos = (
        await db.conversations.find(query, {"_id": 0})
        .sort("timestamp", -1)
        .limit(limit)
        .to_list(limit)
    )
...
async def add_custom_tool(tool: CustomToolCreate):
    ...
    if any(t["name"] == tool.name for t in all_tools):
        raise HTTPException(
            status_code=400, detail=f"Tool '{tool.name}' already exists"
        )
    doc = {
        "name": tool.name,
...
async def list_files_endpoint(
    directory: Optional[str] = None, pattern: Optional[str] = None
):
    query = {}
    if directory:
        query["directory"] = {"$regex": f"^{re.escape(directory)}", "$options": "i"}
...
async def update_file_endpoint(file_id: str, update: FileUpdate):
    ...
    if mode == "append":
        ...
        new_content = file_doc["content"].replace(
            update.find_text, update.replace_text or ""
        )
    else:
...
async def search_files_endpoint(q: str, directory: Optional[str] = None):
    ...
    if directory:
        query["directory"] = {"$regex": f"^{re.escape(directory)}", "$options": "i"}
    files = (
        await db.files.find(query, {"_id": 0, "content": 0})
        .sort("updated_at", -1)
        .to_list(100)
    )
    return files
...
async def update_keywords(update: KeywordsUpdate):
    await db.action_keywords.update_one(
        {}, {"$set": {"keywords": update.keywords}}, upsert=True
    )
