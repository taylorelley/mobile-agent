...
class TestFileRename:
    ...
    def test_01_setup_rename_test_file(self):
        ...
        if response.status_code == 409:
            ...
            existing = next((f for f in files if f["filename"] == "TEST_rename_me.txt"), None)
            if existing:
    ...
    def test_02_rename_file_success(self):
        ...
        print(f"✓ File renamed: {TestFileRename.original_filename} → {data['filename']}")
        
        # Verify persistence with GET
        get_response = requests.get(f"{BASE_URL}/api/files/{TestFileRename.test_file_id}")
    ...
    def test_03_rename_file_updates_path_in_directory(self):
        ...
        if create_resp.status_code == 409:
            files_resp = requests.get(f"{BASE_URL}/api/files")
            files = files_resp.json()
            existing = next((f for f in files if f["path"] == "test_dir/TEST_dir_rename.md"), None)
            if existing:
                requests.delete(f"{BASE_URL}/api/files/{existing['id']}")
            create_resp = requests.post(f"{BASE_URL}/api/files", json={
                "filename": "TEST_dir_rename.md",
                "content": "File in directory",
                "directory": "test_dir"
            })
        ...
        print(f"✓ File renamed in directory: test_dir/TEST_dir_rename.md → {data['path']}")
    ...
    def test_04_rename_file_conflict_409(self):
        ...
        assert rename_resp.status_code == 409, f"Expected 409 conflict, got {rename_resp.status_code}"
    ...
    def test_99_cleanup_rename_test_files(self):
        ...
        if files_resp.status_code == 200:
            files = files_resp.json()
            test_files = [f for f in files if f["filename"].startswith("TEST_") and ("rename" in f["filename"].lower() or "target" in f["filename"].lower())]
...
class TestFileSearch:
    ...
    def test_01_setup_search_test_files(self):
        ...
        for file_data in test_files:
            ...
            if response.status_code == 409:
                ...
                files_resp = requests.get(f"{BASE_URL}/api/files")
                files = files_resp.json()
                path = f"{file_data['directory']}/{file_data['filename']}".strip("/") if file_data['directory'] else file_data['filename']
                existing = next((f for f in files if f["path"] == path), None)
                if existing:
                    # Update content to match test
                    requests.put(f"{BASE_URL}/api/files/{existing['id']}", json={
    ...
    def test_02_search_files_by_name(self):
        ...
        found_test_file = any(f["filename"] == "TEST_shopping_list.txt" for f in shopping_files)
    ...
    def test_03_search_files_by_content(self):
        ...
        assert found_notes or len(results) >= 1, "Search by content should find files containing 'groceries'"
    ...
    def test_05_search_files_case_insensitive(self):
        ...
        print(f"✓ Search is case-insensitive: 'recipe' and 'RECIPE' return {len(results_lower)} results")
    
    def test_06_search_files_with_directory_filter(self):
        """Test GET /api/files/search/query with directory filter"""
        response = requests.get(f"{BASE_URL}/api/files/search/query?q=TEST&directory=documents")
        assert response.status_code == 200
        results = response.json()
        
        # All results should be in documents directory
        for file in results:
            assert "documents" in file.get("directory", "").lower() or "documents" in file.get("path", "").lower()
        print(f"✓ Search with directory filter found {len(results)} files in documents/")
    
    def test_07_search_files_no_results(self):
        """Test GET /api/files/search/query returns empty array for no matches"""
        response = requests.get(f"{BASE_URL}/api/files/search/query?q=nonexistentquery12345xyz")
        assert response.status_code == 200
        results = response.json()
        assert isinstance(results, list)
    ...
    def test_08_search_files_excludes_content_in_response(self):
        ...
        for file in results:
            assert "content" not in file, "Search results should exclude content field for performance"
...
class TestDirectoryTree:
    ...
    def test_01_setup_tree_test_files(self):
        ...
        test_files = [
            ...
            {"filename": "TEST_doc1.txt", "content": "Doc file 1", "directory": "documents"},
            {"filename": "TEST_doc2.md", "content": "Doc file 2", "directory": "documents"},
            {"filename": "TEST_proj1.js", "content": "console.log('hi')", "directory": "projects"},
            {"filename": "TEST_app1.json", "content": '{"test": true}', "directory": "projects/myapp"},
        ]
    ...
    def test_03_tree_includes_root_directory(self):
        ...
        root_dir = next((d for d in tree if d["path"] == "/" or d["name"] == "Root"), None)
        ...
        print(f"✓ Root directory found: {root_dir['file_count']} files, depth={root_dir['depth']}")
    ...
    def test_04_tree_includes_subdirectories(self):
        ...
        has_documents = any("document" in d["name"].lower() or "document" in d["path"].lower() for d in tree)
        has_projects = any("project" in d["name"].lower() or "project" in d["path"].lower() for d in tree)
    ...
    def test_05_tree_has_correct_file_counts(self):
        ...
        for dir_node in tree:
            ...
            assert dir_node["file_count"] == len(dir_node["files"]), \
    ...
    def test_06_tree_has_correct_total_size(self):
        ...
        for dir_node in tree:
            ...
            assert dir_node["total_size"] == calculated_size, \
        ...
        for dir_node in tree:
            for file in dir_node["files"]:
                for field in required_file_fields:
                    assert field in file, f"File missing required field '{field}' in {dir_node['path']}"
    ...
    def test_08_tree_excludes_content_field(self):
        ...
        for dir_node in tree:
            for file in dir_node["files"]:
                assert "content" not in file, "Tree files should exclude content field for performance"
    ...
    def test_09_tree_depth_calculation(self):
        ...
        for dir_node in tree:
            ...
            if path == "/":
                ...
                assert depth == len(segments), f"Depth {depth} should match path segments {len(segments)} for {path}"
    ...
    def test_10_tree_files_sorted_by_filename(self):
        ...
        for dir_node in tree:
            if len(dir_node["files"]) > 1:
                ...
                assert filenames == sorted_filenames, f"Files not sorted in {dir_node['path']}: {filenames}"
