...
class TestFileAPIs:
    ...
    def test_01_create_file_success(self):
        ...
        print(f"✓ File created: {data['path']} (id: {data['id']}, size: {data['size_bytes']} bytes)")
    ...
    def test_04_list_files_returns_all_files(self):
        ...
        print(f"✓ GET /api/files returned {len(files)} files total ({len(test_files)} test files)")
    ...
    def test_07_get_file_by_id_returns_full_content(self):
        ...
        assert "created_at" in data
        assert "updated_at" in data
        print(f"✓ GET /api/files/{TestFileAPIs.test_file_id} returned file with content")
    ...
    def test_10_update_file_overwrite_mode(self):
        ...
        response = requests.put(
            ...
            json={
                "content": "Updated content via pytest",
                "mode": "overwrite"
            }
        )
        assert response.status_code == 200
        ...
        print(f"✓ PUT /api/files/{TestFileAPIs.test_file_id} (overwrite) updated content")
    ...
    def test_99_cleanup_test_files(self):
        ...
        if response.status_code == 200:
            files = response.json()
            test_files = [f for f in files if "TEST_" in f["filename"] or "pytest" in f.get("directory", "")]
...
class TestToolsWithFileTools:
    ...
    def test_get_tools_returns_15_total_tools(self):
        ...
        print(f"✓ GET /api/tools returned {len(builtin_tools)} built-in tools (10 original + 5 file tools)")
    ...
    def test_file_tools_are_present(self):
        ...
        expected_file_tools = ["create_file", "read_file", "edit_file", "delete_file", "list_files"]
        for tool_name in expected_file_tools:
            assert tool_name in tool_names, f"File tool '{tool_name}' not found in tools list"
    ...
    def test_file_tools_have_correct_structure(self):
        ...
        file_tools = [t for t in tools if t["name"] in ["create_file", "read_file", "edit_file", "delete_file", "list_files"]]
...
class TestChatFileOperations:
    ...
    def test_01_chat_create_file_via_natural_language(self):
        ...
        assert "action" in data["routing_decision"] or data["routing_decision"] == "heuristic_requery"
        ...
        if "action_routed" in data["routing_decision"]:
            ...
            if data["tool_calls"]:
                ...
                print(f"✓ Chat created file via natural language, tool: {tool_call.get('tool')}")
        ...
        created_file = next((f for f in files if f["filename"] == "TEST_agent_created.txt"), None)
    ...
    def test_02_chat_read_file_via_natural_language(self):
        ...
        assert "Hello from AI agent" in data["response"] or "TEST_agent_created.txt" in data["response"]
        ...
        if "action_routed" in data.get("routing_decision", ""):
            ...
            if tool_calls:
                ...
                assert tool_result.get("success") == True
                print(f"✓ Chat read file via natural language, content included in response")
    ...
    def test_03_chat_edit_file_via_natural_language_append(self):
        ...
        time.sleep(0.5)
        file_response = requests.get(f"{BASE_URL}/api/files/{TestChatFileOperations.created_file_id}")
    ...
    def test_99_cleanup_chat_test_files(self):
        ...
        if hasattr(TestChatFileOperations, 'created_file_id'):
            response = requests.delete(f"{BASE_URL}/api/files/{TestChatFileOperations.created_file_id}")
            if response.status_code == 200:
