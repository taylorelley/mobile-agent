...
class TestModels:
    ...
    def test_get_models_returns_2_default_models(self):
        ...
        print(f"✓ GET /api/models passed, {len(models)} models found (chat: {len(chat_models)}, action: {len(action_models)})")
...
class TestSettings:
    ...
    def test_get_settings_returns_default_settings(self):
        ...
        print(f"✓ GET /api/settings passed: theme={data['theme']}, agent_name={data['agent_name']}")
...
class TestChat:
    ...
    def test_chat_send_conversational_message_returns_chat_only_routing(self):
        ...
        response = requests.post(
            ...
            json={"message": "What is the weather like?", "session_id": "test-session-1"}
        )
        assert response.status_code == 200
        data = response.json()
        ...
        print(f"✓ POST /api/chat/send (conversational) passed, routing: {data['routing_decision']}")
    ...
    def test_chat_send_action_message_returns_action_routing_with_tool_calls(self):
        ...
        response = requests.post(
            f"{BASE_URL}/api/chat/send",
            json={"message": "Set an alarm for 7am tomorrow", "session_id": "test-session-2"}
        )
        assert response.status_code == 200
        data = response.json()
        assert "response" in data
        ...
        if "action_routed" in data["routing_decision"]:
            ...
            if data["tool_calls"]:
                ...
                print(f"✓ POST /api/chat/send (action) passed, routing: {data['routing_decision']}, tool_calls present")
        else:
            print(f"✓ POST /api/chat/send (action) passed, routing: {data['routing_decision']}")
